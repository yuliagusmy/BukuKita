    ...
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row">
      ...
      <header className="bg-white shadow-sm py-4 px-4 flex justify-between items-center md:hidden">
        <h1 className="text-2xl font-bold text-indigo-700 flex items-center">
          <BookOpen className="w-7 h-7 mr-2 text-indigo-700" /> Buku Kita
        </h1>
        <div className="flex items-center space-x-4">
          <div className="flex items-center space-x-2">
            <span className="text-sm font-medium text-gray-600">Lv.{userStats.level}</span>
      ...
      <aside className="hidden md:flex md:w-64 md:flex-col md:fixed md:inset-y-0">
        <div className="flex flex-col flex-grow bg-gradient-to-b from-[#B3D8FD] via-[#E3F0FF] to-[#E6E6FF] shadow-2xl overflow-y-auto">
          <div className="flex items-center justify-between h-16 flex-shrink-0 px-4">
            <h1 className="text-4xl font-bold text-blue-900 flex items-center drop-shadow">
              <BookOpen className="w-9 h-9 mr-3 text-blue-700" /> Buku Kita
            </h1>
          </div>
          <div className="px-4 py-4 mb-4">
            <div className="bg-white bg-opacity-70 rounded-xl p-4 shadow-md">
              <div className="flex items-center justify-between mb-2">
                <span className="font-medium text-blue-900">{userStats.title}</span>
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.path}
                    to={item.path}
                    className={`flex items-center px-4 py-3 text-sm font-medium rounded-md group transition-colors relative ${
                      isActive
                  ...
                  >
                    {isActive && (
                      <motion.div
                        ...
                        className="absolute left-0 top-0 bottom-0 w-1 bg-indigo-700 rounded-r-full"
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                      />
              ...
              >
                ...
                {isActive && (
                  <motion.div
                    ...
                    className="absolute top-0 w-full h-0.5 bg-indigo-700"
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                  />
